...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(($password == $conpassword) && $exists==false){
        if($type=="Counsellor"){
            ...
            // Prepare and bind
            $stmt = $conn->prepare("INSERT INTO `users` (`firstname`, `secondname`, `email`, `password`, `age`, `gender`, `location`, `phone`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssisss", $firstname, $secondname, $email, $password, $age, $gender, $location, $phone);
            ...
            if ($stmt->execute()) {
                ...
                $stmt = $conn->prepare("INSERT INTO userdata (sno, noofcounsellings, noofreviews) VALUES (?, 0, 0)");
